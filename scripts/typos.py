...
# ΠΡΟΣΟΧΗ ΣΤΑ ΔΙΨΗΦΑ: το «υ» στο ου/αυ/ευ ΔΕΝ προφέρεται /i/, οπότε
# κανείς δεν γράφει «εχοιμε» για «έχουμε». Μια πρώτη εκδοχή εφάρμοζε
# υ→ι παντού και παρήγαγε τέτοια ανύπαρκτα λάθη — που μετά μετριόνταν
# ως αποτυχίες του διορθωτή και έδιναν λάθος εικόνα για το πού αξίζει
# να επενδύσουμε. Τα lookbehind/lookahead τα προστατεύουν.
PHON = [
    (r"οι", "ι"), (r"ει", "ι"), (r"(?<![αεο])υ(?![ιν])", "ι"),
    (r"η", "ι"), (r"(?<![αεο])ι", "η"),
    (r"(?<![αεο])αι", "ε"), (r"(?<![αο])ε(?![υι])", "αι"),
    (r"ω", "ο"), (r"(?<![α-ω])ο(?![ιυ])", "ω"),
]
